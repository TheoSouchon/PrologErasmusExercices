% La liste vide est considérée comme triée
insert_sorted(X, [], [X]).
% Si l'élément est plus petit que la tête de la liste, on l'insère en tête
insert_sorted(X, [H|T], [X,H|T]) :- X =< H.
% Sinon, on insère récursivement l'élément dans la queue de la liste
